...
The medical large language model, PULSE, is released in the OpenMEDLab. It is trained and then instruction-tuned (in reinforcement learning) using two datasets, i.e., one with 4,000,000 multi-linguistic data samples (equivalent to approximately 9.6 billion tokens) and the other with labels (expert-confirmed scores and ranks of responses generated by the model). A self-evaluation prompt is added to the reward model training, and the standard PPO framework is further optimized for better performance. It also demonstrates a downstream application of the PULSE in understanding the literature of SARS-COV-2 Plugins for the downstream applications are under development. Quantized, updated versions and larger models will be released soon (see projects in [Medical Large Language Models](https://github.com/openmedlab/.github/blob/main/profile/README.md#medical-large-language-models)).  



XXX is fine-tuned on PULSE specifically for SARAS-COV-2
